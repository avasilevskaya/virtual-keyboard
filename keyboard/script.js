...
const keyboard = {
    ...
    keyList.forEach((key) => {
      ...
      switch (keyCode) {
        case 'Backspace':
          ...
          keyElement.addEventListener('click', () => {
            const posStart = this.textInput.selectionStart;
            const posEnd = this.textInput.selectionEnd;
            if (posStart !== posEnd) {
  ...
  insertToInput(keyCode) {
    let txt;
    const key = keyList.find((arg) => arg.keyCode === keyCode);
    if (key) {
      const keyObj = key[this.properties.lang];
      const keyValueNormal = keyObj.normal;
      const keyValueCaps = keyObj.caps;
...
document.addEventListener('keydown', (event) => {
  ...
  const animatedKey = document.getElementById(event.code);
  if (animatedKey) {
